...
void solve(fftw_plan fft, fftw_plan ifft,
  ...
  for (nn=0; nn<DFFT_LEN; nn++){
    transfer[nn][0] = r0 / (r0*r0 + (freqs[nn] + f0));
    transfer[nn][1] = - (freqs[nn] + f0)  / (r0*r0 + (freqs[nn] + f0));
    temp[0] = AUX[nn][0];
    temp[1] = AUX[nn][1];
    AUX[nn][0] = (temp[0] * transfer[nn][0] - temp[1] * transfer[nn][1]) / DFFT_LEN;
    AUX[nn][1] = (temp[1] * transfer[nn][0] + temp[0] * transfer[nn][1]) / DFFT_LEN;
  }
