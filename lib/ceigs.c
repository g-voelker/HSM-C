...
void comqr2(n,low,high, ortr,orti,hr,hi, wr,wi, zr,zi)
   int n,low,high;
   double *ortr,*orti,*wr,*wi;
   double **hr,**hi,**zr,**zi;
/*
...
*/ 
{
   ...
   double si,sr,ti,tr,xi,xr,yi,yr,zzi,zzr;
   double norm,tst1,tst2;
...
void corth(n,low,high, ar,ai, ortr,orti)
   int n,low,high;
   double *ortr,*orti;
   double **ar,**ai;
/*
...
void cbabk2(n,low,high, scale,m, zr,zi)
   int n,m,low,high;
   double *scale;
   double **zr,**zi;
/*
...
void cbal(n, ar,ai, low,high, scale)
   int n;
   int *low,*high;
   double *scale;
   double **ar,**ai;
...
*/
{
   ...
   l = n;
   for (;;) {
      if (DBGFLG>=2) {printf("CBAL: main loop %d\n",mlcnt++);fflush(NULL);}
      for (j=l;j>=1;j--) {
         zflg1 = 0;
         for (i=1;i<=l;i++) 
      ...
      if (zflg1==0) {
         ...
         sbcnt=1;
         for (;;) {
            if (DBGFLG>=2) {printf("CBAL: sub loop %d\n",sbcnt++);fflush(NULL);}
   ...
   if (zflg1>0) {
      ...
      while (noconv) { /* iterative loop for norm reduction */
         if (DBGFLG>=2) {printf("CBAL: while loop %d\n",wcnt++);fflush(NULL);}
         noconv = FALSE;
         for (i=k;i<=l;i++) {
            c = 0.0;
            r = 0.0;
...
*/
{
   ...
   if (DBGFLG>=1) { printf("CG (DEBUG): calling cbal ...\n"); fflush(NULL); }
   ...
   */
   if (DBGFLG>=1) { printf("CG (DEBUG): calling corth ...\n"); fflush(NULL); }
   corth(n,is1,is2,ar,ai,fv2,fv3);

   /* find both eigenvalues and eigenvectors */
   if (DBGFLG>=1) { printf("CG (DEBUG): calling comqr2 ...\n"); fflush(NULL); }
   comqr2(n,is1,is2,fv2,fv3,ar,ai,wr,wi,zr,zi);
   
   if (DBGFLG>=1) { printf("CG (DEBUG): calling cbabk2 ...\n"); fflush(NULL); }
   cbabk2(n,is1,is2,fv1,n,zr,zi);

   if (DBGFLG>=1) { printf("CG (DEBUG): ending ...\n"); fflush(NULL); }
   free_dvector(fv1,1,n);
   free_dvector(fv2,1,n);
   free_dvector(fv3,1,n);
}
